...
# =====================
# SAVE RANDOM FOREST MODEL (OS PATH)
# =====================
from joblib import dump, load

dump(model, os.path.join(MODEL_DIR, "Mining.joblib"))
...
def cv_table(model, X, y, cv=5, random_state=42):
    ...
    mean_row = pd.DataFrame({"Fold": ["Mean"], "RMSE": [df["RMSE"].mean()], "R2": [df["R2"].mean()], "MAE": [df["MAE"].mean()]})
    std_row = pd.DataFrame({"Fold": ["Std"], "RMSE": [df["RMSE"].std()], "R2": [df["R2"].std()], "MAE": [df["MAE"].std()]})
    return pd.concat([df, mean_row, std_row], ignore_index=True)
...
def train_test_table(model, X_train, y_train, X_test, y_test):
    ...
    rows = []
    for split, y, p in [("Train", y_train, pred_tr), ("Test", y_test, pred_te)]:
        rows.append({"Split": split, "RMSE": np.sqrt(mean_squared_error(y, p)), "R2": r2_score(y, p), "MAE": mean_absolute_error(y, p)})
    return pd.DataFrame(rows)
...
# =====================
# SAVE XGBOOST MODEL 1
# =====================
dump(best_xgb, os.path.join(MODEL_DIR, "Mining_XGBoost_Model.joblib"))
...
# =====================
# SAVE XGBOOST MODEL 2 (EARLY STOP)
# =====================
dump(final_pipe, os.path.join(MODEL_DIR, "Mining_XGBoost_cv_earlystop.joblib"))
