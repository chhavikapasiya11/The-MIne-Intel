...
/* ----------- LabeledInput (kept outside component) ----------- */
function LabeledInput({ label, keyName, placeholder, helper, type = 'number', step, full = false, value, onChange }) {
  return el(
    ...
    helper
      ? el('div', { style: { fontSize: 13, color: 'var(--muted)', marginTop: 4 } }, helper)
      : null
  );
}
...
export default function PredictPage() {
  ...
  async function handlePredict() {
    ...
    const body = {
      ...
      depthOfCover: Number(form.depthOfCover),
      intersectionDiagonal: Number(form.intersectionDiagonal),
      miningHeight: Number(form.miningHeight)
    };
    ...
    try {
      ...
      if (payload) {
        if (payload.rate) rate = payload.rate;
        else if (typeof payload.prediction === 'number') {
          rate = `${Number(payload.prediction).toFixed(2)}%`;
        } else if (typeof payload.prediction === 'string') {
          rate = payload.prediction;
        }
      }
  ...
  return el(
    'div',
    { className: 'page page-enter', style: { padding: '28px 18px' } },
    ...
    el(
      ...
      el(
        ...
        el(LabeledInput, {
          ...
          keyName: 'cmrr',
          placeholder: '50',
          helper: 'Coal Mine Roof Rating (0–100)',
          step: '1',
          value: form.cmrr,
          onChange: (e) => update('cmrr', e.target.value)
        }),
        el(LabeledInput, {
          label: 'PRSUP',
          keyName: 'prsup',
          placeholder: '40',
          helper: 'Percentage of roof support (0–100)',
          step: '1',
          value: form.prsup,
          onChange: (e) => update('prsup', e.target.value)
        }),
        el(LabeledInput, {
          label: 'Depth Of Cover (m)',
          keyName: 'depthOfCover',
          placeholder: '200',
          helper: 'Depth in meters',
          step: '0.1',
          value: form.depthOfCover,
          onChange: (e) => update('depthOfCover', e.target.value)
        }),
        el(LabeledInput, {
          label: 'Intersection Diagonal (m)',
          keyName: 'intersectionDiagonal',
          placeholder: '5',
          helper: 'Diagonal in meters',
          step: '0.1',
          value: form.intersectionDiagonal,
          onChange: (e) => update('intersectionDiagonal', e.target.value)
        }),
        el(LabeledInput, {
          label: 'Mining Height (m)',
          keyName: 'miningHeight',
          placeholder: '2.5',
          helper: 'Height in meters',
          step: '0.01',
          full: true,
          value: form.miningHeight,
          onChange: (e) => update('miningHeight', e.target.value)
        })
      ),
        ...
        ? el(
            ...
            el(
              ...
              el(
                ...
                'Predicted Roof Fall Rate:'
              ),
              el(
                'div',
                { style: { marginTop: 6, fontSize: 22, fontWeight: 900, color: 'white' } },
                result.rate
              )
            )
          )
...
function ChatWidget({ updateForm }) {
  ...
  const [messages, setMessages] = useState([
    {
      from: 'bot',
      text: 'Hi — I can help run predictions, explain parameters or show charts. Ask me anything about roof fall rate.'
    }
  ]);
  ...
  function send(txt) {
    ...
    /* Update form only if features exist */
    if (updateForm) {
      if (features.CMRR !== null) updateForm('cmrr', features.CMRR);
      if (features.PRSUP !== null) updateForm('prsup', features.PRSUP);
      if (features.depth_of_cover !== null)
        updateForm('depthOfCover', features.depth_of_cover);
      if (features.intersection_diagonal !== null)
        updateForm('intersectionDiagonal', features.intersection_diagonal);
      if (features.mining_height !== null)
        updateForm('miningHeight', features.mining_height);
    }
  ...
  return el(
    'div',
    { className: 'chat-widget', style: { position: 'fixed', bottom: 20, right: 20, width: 300, zIndex: 999 } },
    ...
    open &&
      el(
        ...
        el(
          'div',
          { className: 'chat-log', role: 'log', 'aria-live': 'polite' },
          ...messages.map((m, i) =>
            el(
              'div',
              {
                ...
                style: {
                  ...
                  background:
                    m.from === 'bot' ? 'rgba(0,0,0,0.3)' : 'rgba(255,255,255,0.1)',
                  padding: '6px 10px',
                  borderRadius: 8,
                  fontSize: 14,
                  lineHeight: '1.4'
        ...
        el(
          ...
          el(
            'button',
            { type: 'submit', className: 'btn btn-primary', style: { padding: '6px 10px' } },
            'Send'
          )
        )
      )
